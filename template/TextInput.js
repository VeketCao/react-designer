...
$(document).ready(function(){
    ...
    var bindEvent = function () {
        ...
        $('input[type="text"]').unbind("keyup").bind('keyup',function () {
            if ($(this).attr('name') == 'title' || $(this).attr('name') == 'description' || $(this).attr('name') == 'validMsg') {
                ...
                // 调用全局保存控件编辑属性方法
                saveComponentParmFunc.saveChangeButton();
            } else if ($(this).attr('name') == 'minLength' || $(this).attr('name') == 'maxLength') {
                isValidate($(this));
                // 调用全局保存控件编辑属性方法
                saveComponentParmFunc.saveChangeButton();
            } else {
    ...
    var isValidate = function (obj) {
        ...
        if (isOK) {
            if ((obj.attr('name') == 'maxLength' && parseInt(obj.val()) <= parseInt($('input[name="minLength"]').val())) || (obj.attr('name') == 'minLength' && parseInt(obj.val()) >= parseInt($('input[name="maxLength"]').val()))) {
