...
window.ClickAway = {
    bindClickAway: function (clickAwayEvent, target) {
        var fn;
        if (clickAwayEvent && target) {
            fn = this.getEvent(clickAwayEvent, target);
        }
        else {
    ...
    unbindClickAway: function (clickAwayEvent, target) {
        var fn;
        if (clickAwayEvent && target) {
            fn = this.getEvent(clickAwayEvent, target);
        }
    ...
    registerClickAway: function (onClickAway, target) {
        this.clickAwayTarget = target;
        this.onClickAway = onClickAway;
    },
    getEvent: function (event, target) {
        var func = function (e) {
            if (target !== e.target && !ClickAway.isDescendant(target, e.target)) {
                event();
            }
        };
        return func;
    },
    getClickAwayEvent: function () {
        var func = this._clickAwayEvent;
        var clickTarget = this.clickAwayTarget;
        if (!func) {
            func = function (e) {
                if (clickTarget !== e.target && !ClickAway.isDescendant(clickTarget, e.target)) {
                    if (ClickAway.onClickAway) {
                        ClickAway.onClickAway();
                    }
                }
            };
            this._clickAwayEvent = func;
        }
        return func;
    },
...
/**current js path**/
window.getCurrJsPath=function(name){var js=document.scripts;var jsPath;for(var i=0;i<js.length;i++){if(js[i].src.indexOf(name)>-1){jsPath=js[i].src.substring(0,js[i].src.lastIndexOf("/")+1);break}}return jsPath};
/*deep clone object*/
...
/**http**/
window.http=function(url,param,options){param=param||{};var dtd=$.Deferred();var opts=_.defaults(options||{},{dataType:"json",type:"GET",cache:false,timeout:120000});opts=_.extend(opts,{data:param,url:url});$.ajax(opts).done(function(rtn){dtd.resolve(rtn)}).fail(function(rtn){console.log("url",url);console.error("error",rtn);dtd.reject(rtn)}).always(function(){});return dtd.promise()};
window.dataProcess4Component=function (data) {
    try{
...
(function () {
    ...
    window.TOKEN_NAME="";
    var WD_CONFIG = {
        T_API_URL: 'http://mipnpre2.midea.com:10001',
        T_COMPONENT_URL:'http://mipnpre2.midea.com:10001',
        T_ESB_API:'http://mipnpre2.midea.com:10001',
        T_TOKEN_NAME:'mideatest_sso_token',
        N_API_URL: 'http://cip2sit.midea.com:10001/cipman',
        N_COMPONENT_URL:'http://cip2sit.midea.com:10001/resource/component/file/',
        N_ESB_API:'http://cip2sit.midea.com:10001/esbrt/pre.do?alias=',
        N_TOKEN_NAME:'mideatest_sso_token',
        IS_T_ENV: true, // test
        IS_N_ENV: false // normal
    };
    var __initEnvParam = function () {
        if (WD_CONFIG.IS_T_ENV) {
            TOKEN_NAME = WD_CONFIG.T_TOKEN_NAME;
            TEMP_URL = WD_CONFIG.T_API_URL;
            ESB_URL = WD_CONFIG.T_ESB_API;
            window.COMPONENT_URL = WD_CONFIG.T_COMPONENT_URL;
        } else if(WD_CONFIG.IS_N_ENV) {
            TOKEN_NAME = WD_CONFIG.N_TOKEN_NAME;
            TEMP_URL = WD_CONFIG.N_API_URL;
            ESB_URL = WD_CONFIG.N_ESB_API;
            window.COMPONENT_URL = WD_CONFIG.N_COMPONENT_URL;
        }
    };
    __initEnvParam();
    // api object
    var JR_API = {
        search_single_form: TEMP_URL + 'mipFormEngine/form/getByFdId',
        search_form: TEMP_URL + 'mipFormEngine/form/search',
        save_form: TEMP_URL + 'mipFormEngine/form/save'
    };

    window.WD_API = {
        getDataSourceApi:ESB_URL
    };
    window.setcookie = function(name,value){
        ...
        document.cookie = name + "="+ escape (value) + ";expires=" +
            exp.toGMTString()+";path=/"+";domain=.midea.com";
    };
    ...
    window.getDataSourceApi= function (params,cb,callback) {
        var getRes=function(){
            var dt = new Date();
            dt.setMinutes(0, 0, 0);
            var timestamp = Date.parse(dt);
            var tid = "{\"tm\":\""+timestamp+"\",\"tid\":\"21232F297A57A5A743894A0E4A801FC3\"}";
            var hex_md5_value=hex_md5(tid);
            return  hex_md5_value;
        };
        var sso_token_value = getcookie(TOKEN_NAME);
        var urlFunc = WD_API.getDataSourceApi+params.value+"&tenantId="+getRes()+"&"+params.param+"&"+TOKEN_NAME+"="+encodeURI(sso_token_value);
        http(urlFunc,{},{dataType:"jsonp", jsonp: callback || "jsonpcallback"}).done(function (rtn) { cb(rtn); }).fail(function(rtn){ cb(rtn,{msg:'服务错误',status:-9999});console.log("服务错误")});
    };
})();
/**valid**/
window.validtion=function($form){
