...
window.CheckBox = React.createClass({
    ...
    changeHandle:function(e){
        var values = _.map($(this.getDOMNode()).find('[type="checkbox"][name='+this.state.name+']:checked'),function (element) {
            return $(element).val();
        })
        this.setState({
                        ...
                        <div className="form-component-group">
                            ...
                            <div>
                            {
                                _.map(typeof _options =="string" ? _options.split(','):_options,function (o,index) {
                                    if (o != '') {
                                        return (
                                            <label className="el-component el-checkbox" style={hoz} key={index}><span className="el-checkbox-input"><input className="el-checkbox-original" type="checkbox" name={this.state.name} onClick={this.changeHandle} value={o} /><i></i></span><span className="el-label">{o}</span></label>
                                        )
                                    } else {
                                        return (
                                            <label className="el-component el-checkbox" style={hoz}><span className="el-checkbox-input"><input className="el-checkbox-original" type="checkbox" name={this.state.name} onClick={this.changeHandle} value={'选项'} /><i></i></span><span className="el-label">选项</span></label>
                                        )
                                    }
